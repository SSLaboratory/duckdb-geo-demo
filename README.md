...
# Run (persistent data in /tmp/data)
...
4. **View re-registration** — iterates `_datasets` and recreates DuckDB views pointing to existing parquet files (views are lost on restart since they live in-memory)
5. **Static files** — mounts the frontend at `/`
6. **Ready** — Uvicorn starts accepting requests on port 8000
...
| `DELETE` | `/api/datasets/{name}` | Delete dataset (removes view, metadata, and parquet file) |
...
| `POST` | `/api/builtins/load` | Download and ingest (`{"dataset": "ne-countries-110m"}`) |
